...
class LBUNaviController: UINavigationController {
    ...
    func setupLayout() {
        guard let interactionGes = interactivePopGestureRecognizer,
            let targetView = interactionGes.view , 
            let internalTargets = interactionGes.value(forKey: "targets") as? [NSObject],  
...
extension LBUNaviController: UIGestureRecognizerDelegate {
