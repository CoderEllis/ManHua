...
class LBUCateController: LBUBaseController {
    ...
    private lazy var searchButon: UIButton = {
        ...
        searchButon.frame = CGRect(x: 0, y: 0, width: screenWidth - 20, height: 30)
        searchButon.backgroundColor = UIColor.black.withAlphaComponent(0.1)
        searchButon.layer.cornerRadius = 15
        searchButon.setTitleColor(UIColor.white, for: .normal)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            for tab in model.extra?.tabList ?? [] {
                guard let tabTitle = tab.tabTitle else { return }
                titles.append(tabTitle)
                vcs.append(LBUComicListController(argCon: tab.argCon, argName: tab.argName, argValue: tab.argValue))
            }
