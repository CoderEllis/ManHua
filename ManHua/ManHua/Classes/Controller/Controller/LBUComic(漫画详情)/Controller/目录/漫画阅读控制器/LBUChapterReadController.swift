...
class LBUChapterReadController: LBUBaseController {
    ...
    func loadData(with index: Int, isPreious: Bool, needClear: Bool, finished: ((_ finished: Bool) -> ())? = nil) {
    ...
    lazy var collectionView: UICollectionView = {
        ...
        collectionView.uFoot = URefreshAutoFooter { [weak self] in
            ...
            self?.loadData(with: nextIndex, isPreious: false, needClear: false, finished: { [weak self] (finish) in
                self?.previousIndex = nextIndex + 1
            })
        }
    ...
    override func setupLayout() {
        ...
        backScrollView.snp.makeConstraints{ $0.edges.equalTo(view.usnp.edges)}
...
// MARK: - 点击事件
extension LBUChapterReadController {
    @objc func tapAction() {
...
extension LBUChapterReadController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        guard let image = chapterList[indexPath.section].image_list?[indexPath.row] else { return .zero }
