...
//        cycleScrollView.lldidSelectItemAtIndex = didSelectBanner(index:)
        cycleScrollView.lldidSelectItemAtIndex = { index in
            self.didSelectBanner(index: index)
        }
        
//        cycleScrollView.lldidSelectItemAtIndex = { self.didSelectBanner(index: $0) }
        return cycleScrollView
    }()
    
    private lazy var sexTypeButton: UIButton = {
        let sn = UIButton(type: UIButton.ButtonType.custom)
        sn.setTitleColor(UIColor.black, for: UIControl.State.normal)
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.uempty = UEmptyView(verticalOffset: -(collectionView.contentInset.top), tapClosure: { 
            self.setupLoadData(false)
        })
...
extension LBUHomeCommentController: UICollectionViewDataSource, UCollectionViewSectionBackgroundLayoutDelegateLayout {
        ...
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            headerView.moreActionClosure { [weak self] in
                if comicList.comicType == .thematic {
                    let vc = LBUPageController(titles: ["漫画","次元"], vcs: [LBUSpecialController(argCon: 2),LBUSpecialController(argCon: 4)], pageStyle: LBUPageStyle.navgationBarSegment)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if comicList.comicType == .billboard {
            ...
            if item.linkType == 2 {
                guard let url = item.ext?.compactMap({ return $0.key == "url" ? $0.val : nil}).joined() else { return }
// compactMap 筛选 ? 匹配组  joined 返回：一个连接字符串。
/*
[ManHua.LBUExtModel(key: Optional("url"), val: Optional("http://m.u17.com/wap/cartoon/show?cartoon_id=1")), 
 ManHua.LBUExtModel(key: Optional("cover"), val: Optional("http://image.mylife.u17t.com/2017/12/20/1513738638_pWE9248WdEHt.jpg")), 
 ManHua.LBUExtModel(key: Optional("title"), val: Optional("《十万个冷笑话》动画")), 
 ManHua.LBUExtModel(key: Optional("content"), val: Optional("")), 
 ManHua.LBUExtModel(key: Optional("description"), val: Optional("")), 
 ManHua.LBUExtModel(key: Optional("canToolBarShare"), val: Optional("false"))]
                 
 pringLog(url) http://m.u17.com/wap/cartoon/show?cartoon_id=1
*/
                let vc = LBUWebController(url: url)
                navigationController?.pushViewController(vc, animated: true)
            } else {
